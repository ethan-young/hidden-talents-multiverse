...
file_list <- list.files("scripts", pattern = ".R$", full.names = T, recursive = T) %>% 
  map_df(function(x){
    ...
    tibble(
      ...
      input     = input2 %>% str_split(" ", simplify = T) %>% str_remove_all(".*/") %>% str_c(collapse = "<br> ") %>% ifelse(. == " ","",.),
      ...
      output    = output2 %>% str_split(" ", simplify = T) %>% str_remove_all(".*/") %>% str_c(collapse = "<br> ") %>%  ifelse(. == "NA","",.)
    )
  }) 
```
